...
import { View, Text, Button, Image, ScrollView, StyleSheet, StatusBar } from 'react-native';
...
const Dashboard = () => {
    ...
    const fetchPendingUsers = async () => {
        try {
            const response = await axios.get(Constants.expoConfig.extra.IP_ADDRESS + '/admin/pending-users');
    ...
    const updateUserStatus = async (userId, status) => {
        try {
            await axios.post(Constants.expoConfig.extra.IP_ADDRESS + '/admin/update-status', { userId, status });
                    ...
                    users.map(user => (
                        <View key={user._id} style={styles.card}>
                            ...
                            <View style={styles.buttonContainer}>
                                <Button title="Approve" onPress={() => updateUserStatus(user._id, 'approved')} />
                                <Button title="Reject" onPress={() => updateUserStatus(user._id, 'rejected')} />
                            </View>
                        </View>
                    ))
                )}
