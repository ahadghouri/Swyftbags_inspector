...
const PickedUp = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch trips with status "reached inspector 1" and start field equal to adminCity
  ...
  const handleDispatched = async (tripId) => {
    try {
      const token = await AsyncStorage.getItem("token");

      // Update trip status to "dispatched" when received
      await axios.put(
      ...
      {loading ? (
        ...
        trips.map((trip) => (
          <View key={trip._id} style={styles.card}>
            ...
            <Button title="Dispatched" onPress={() => handleDispatched(trip._id)} />
          </View>
        ))
      )}
